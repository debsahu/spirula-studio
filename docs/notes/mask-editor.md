...
| load_picture 8K -> 1024 / 4096 / 1080p -> 1024, memory9 | 116.8 / 146.2 / 10.5 ms | stb's buffers in place, one run after the page hook was dropped (`measurements/memory9/bench8_nohook_run1.txt`). Before: 120.0 / 138.2 / 9.7 |
| slideshow pool, memory9 (**the criterion 8 gate**) | 16.6 fps (2 threads) / 13.4 (2) / 369.8 (4) / 426.6 (4) | 8K -> 1024, 8K -> 4096, 1080p -> 1024, 1080p -> 4096; one run, each row PASS; longest waits 120.5 / 147.7 / 7.5 / 9.1 ms. Before, all at 4 threads: 31.1 / 20.1 / 379.4 / 405.8 |
| resident memory WHILE PLAYING, 8K, memory9 (**the criterion 9 gate**) | +244 MB control; +305 SAM | medians of three fresh launches each: control +562 / +154 / +244, SAM +298 / +305 / +421; bar 600: **PASS**. See "memory9" |
...
does (both call the identical function, `MaskPanel.cpp:197` and `:636`), and
...
(`MaskSession.cpp:855-866`, `if (r.empty()) return {};`) and `upload_rect`
...
readout (`MaskPanel.cpp:705`), read before the series and after every one of
...
2. **Undo + Save**: `cmp base vs masks/f0000.png` exited 0 (byte-identical to
   ...
   worked. `MaskPanel.cpp:197` (`ui::Button(msg::undo)`) and `:636`
...
- [x] **Left/Shift+drag tints drop (red-ish); Ctrl+drag tints keep
  ...
  at release" is read from source (`MaskPanel.cpp:497-498` samples
  `io.KeyShift` / `io.KeyCtrl` into `ViewportInput` on the frame being drawn,
  and `:533` hands those to `paint_now` on the frame the stroke commits)
...
- [x] **`<`, `>` and the slider change frames; a dirty frame reports "Saved"
  ...
  **not** independently exercised; `go_to()` is the single function behind
  all three (`MaskPanel.cpp:229`, `:234`, `:237`), so this is inferred from
  the one call tested.
...
`_status` as it publishes the loaded frame (`MaskSession.cpp:323`, inside the
`enqueue` lambda that starts at `:297`), and `pump()` calls
...
`draw_status` can emit.** Enumerating it (`MaskPanel.cpp:686-735`), the eight
...
and `status_base_missing` (`MaskPanel.cpp:711-714`), one `TextDisabledWrapped`
...
canvas at `px(64.0f)`, and `draw_status` (`MaskPanel.cpp:686-735`) draws its
...
(`MaskSession.h:164-165`) address the one float. The slider, `[`/`]` and
...
`MaskPanel.cpp:244-258`, on the **frame-navigation row**, shown only under the
...
(`MaskPanel.cpp:259-262`), plus a `help_on_hover` sentence of the kind `save` /
...
`MaskPanel.cpp:435-443`. The bare wheel is already the zoom, and Shift/Ctrl
...
`wheel_brush` join it in `MaskSession.cpp:873-898`, the file
...
- **The open frame's pixels are co-owned.** `_rgb` is a
  `shared_ptr<const vector<uint8_t>>` (`MaskSession.h:481`) and a job holds
...
active (`SetItemKeyOwner`, `MaskPanel.cpp:395-400`), which is what stops
...
every mode (`MaskPanel.cpp:733-734`). `state_json` reports `mask_peek`,
...
### Task 9: find missing (2026-09-23)

A scan thread starts at the end of `open()` and computes each frame's kept
fraction under the folder's convention (`_mask_flipped`), through
`mask_edits/kept.json` keyed by fingerprint. Row D holds First, Previous
missing, Next missing, Last, the band (Min/Max kept %, default 5 and 98) and
`Scanning masks: n / N`, then `Missing: n`. The keys are Left/Right, PageUp/
PageDown (ten), Home/End, `M` and `Shift+M`; their list is the frame slider's
tooltip, not a strip line. Row D's buttons, `<`, `>` and the frame slider wait
for the worker, are greyed while the slideshow plays, and stand down while a
shape or pen path is half drawn (`shape_open()`, tooltip `nav_locked`), as the
keys do; the keys also stand down for Ctrl. A band field clamps against the
other end and never moves it (`band_edit`): typing Min above Max stops at Max.

**Only the worker writes masks, and the scan never publishes a read a worker
job overlapped.** `worker_main` makes `_job_seq` odd for the length of each
job. The scan reads the sequence, reads the mask (fingerprint, then decode),
and publishes under `_mu` only if the sequence is unchanged; otherwise it drops
the frame's cache entry and reads it again. A job that rewrote the mask
refreshes that frame's health itself, so without the check the scan's older
read would overwrite the newer value (`test_scan_yields_to_a_write` holds the
scan between read and publish and runs a propagate). Every cached entry is
therefore a fingerprint and the fraction of those same bytes, and a mask
changed since it was cached misses by fingerprint. Writers outside the app (a
CLI `spirula sam` run) are not seen until the next scan.

**The health follows every mask write the session makes, from the job that
wrote it:** a save (its own job, not `pump()`: `go_to()` and Play save and drop
the document before the save lands, so a pump-side refresh left the frame just
fixed counted missing), propagate (every attempted target, and the source when
the job saved it) and its undo (every attempted target),
Revert frame, Revert all (the index's keys, read before `revert_all`), and a
load that rebased a regenerated mask and wrote a new composite. The last is
not in the plan: a re-run between sessions is scanned as the raw new mask, and
only the load that rebases it writes the composite the trainer then reads.

**`kept_fraction_of` skips anything that is not a regular file.** A FIFO at a
mask path blocked the scan in `fopen` until a writer came, and `close()` then
blocked joining it. The slideshow tests put FIFOs at mask paths, so the suite
hung under one mutant; the scan also satisfied those tests' "a reader is
waiting" probe in place of the decode they meant to catch.

**Spec 9.2's second arm, "a layer but no base (5.4)", needs no code.** 5.4's
base is `masks/<key>.png`: with it absent the first arm already holds,
whatever layers exist (`test_find_missing_layer_without_mask`). A corrupt PNG
is missing to the scan; the load that `M` starts fails and the status line
names the file (`Could not read ...`), so the two agree that the frame is
broken and differ only in wording, because no document opens. The next `M`
searches from the frame last asked for, not `_idx`, which a failed load leaves
on the frame before: searching from there landed on the corrupt frame again,
every time.

The scan's cache write creates `mask_edits/` on a dataset nobody has
corrected. `workspace_artifacts` does not list it (spec 5.2). `KeptCache`
refuses a relative or empty root: a mutant close() that cleared `_layer_root`
under the running scan made its final save write `kept.json` into the
working folder, the repository root under the test runner.

`close()` stops the scan before it clears `_frames` (Decision 10);
`test_close_mid_scan` holds the scan mid-dataset and checks the frames are still
there after close() has had 200 ms, so a late stop fails by name without TSan.
TSan confirms it as a race too, and is the only check for races in general:
`-O1 -g -fsanitize=thread` over `mask_doc_test`'s sources, private `TMPDIR`
(the reviewer's recipe, 0 warnings on the suite; control: the scan's publish
lock removed gives one race).

The band's two floats are UI-thread only; the scan never reads them. Declared
gap: the scan's "one failed cache write ends the periodic writes" needs more
than 64 frames and is not tested. The keys were verified at rest, one press at
a time: a held key on an 8K dataset drops presses while each frame loads,
because `handle_keys` does not run without a document.

| check (M5 Pro, offscreen 1600x950, the workflow bench, `f0001.png` deleted) | observed |
|---|---|
| cold scan, no `kept.json` (3 decodes, 1 absent) | `mask_scan_ms` 305, `Missing: 1` |
| reopen | 143 ms, `kept.json` md5 and mtime unchanged |
| `M` from frame 1 | frame 2 of 4, `f0001`, "No mask on disk for this frame." |
| `Shift+M`, then `M` from there | "No missing frame in that direction." both ways |
| Left, Left, Right, Right, Home, End, PageUp, PageDown | 2, 1, 2, 3, 0, 3, 0, 3; `nav_visible` false throughout |
| pen (`I`), two anchors, Left, then `M` | frame and 2 anchors unchanged; Esc, then Left steps |
| slider hover | the key list as its tooltip |
| playing | First, Previous missing, Next missing, Last drawn greyed |
| flipped: `f0000.png` 3% white, same session, Advanced `flip_mask` on | unflipped `Missing: 2`, Kept 3.0%; flipped `Missing: 1`, Kept 97.0% = kept.json's 0.970052 |
...
decoder): (a) fewer decoders on malloc; (b) a preview cache in the dataset,
rejected, it passes only on a second launch and leaves files in the
operator's dataset; (c) stb's large buffers from `mmap`, unmapped at free;
(d) `MallocLargeCache=0` in the environment, undocumented, not chosen. The
plan chose (c) + (a); (c) was built, measured, and dropped (below).
...
- `load_picture` boxes stb's RGB straight into the caller's picture, frees
  ...
  and the photo and mask never coexist. stb is on plain malloc.
...
**The page hook, tried and dropped.** `STBI_MALLOC` routed blocks of 1 MiB
and up to `mmap`/`munmap` on the decoder threads only (a thread-local opt-in,
a 64-byte header per block, `SS_STB_PAGE_ALLOC=0` as the A/B knob;
`81e13893`, removed by `ae77f16c`). With it, #9 read control +481 / +489 /
+489 and SAM +227 / +1046 / +227; with the knob off, +243 / +563 / +243.
`vmmap` at 10 s with the hook: live `MALLOC_LARGE` 19.8 MB, `(empty)` 410.2 MB
in all three control launches. That is what the editor freed at Play and the
churn of opening the frame, and mapped decode buffers cannot recycle it. A
process-wide build read +435 / +435 / +434. The operator dropped it: a smaller
upstream change, and in the app it made the reading worse. Raw files
`measurements/memory9/{C1..3,S1..3,C0-1..3,diag_processwide_C1..3}.raw`.

**The readings, as shipped.** `tools/mask_editor_checks/memory9.sh`, three
fresh launches per arm, interleaved, each on a fresh copy of the 8K bench,
normal priority, `-NSAppSleepDisabled YES`, dataset through the in-app folder
dialog, binary `3395ac1a` built from `a1cb5613`. `threads` read 2 and
`sam_vram_mib` -1.0 at every playing sample. Raw files
`measurements/memory9/nohook_{C1..3,S1..3}.raw`.

| arm | 10 s readings (MB over baseline) | median | predicted (the hook-off A/B) | vs bar 600 |
|---|---|---|---|---|
| C, control | +562 / +154 / +244 | **+244** | +243 / +563 / +243 | **PASS** |
| S, SAM loaded first | +298 / +305 / +421 | **+305** | none recorded without the hook | **PASS** |

The control reproduces the A/B to 1 MB at the median and to the spread (C1's
+562 against C0-2's +563). C1 is a residue sample by the plan's rule: live
62.0 MB, `(empty)` 466.4 MB. Every other sample has live <= 149 MB.

**Criterion #8**, one run after the drop (`bench8_nohook_run1.txt`):
8K -> 1024 **16.6 fps** (2 threads), 8K -> 4096 **13.4** (2), 1080p -> 1024
**369.8** (4), 1080p -> 4096 **426.6** (4); longest waits 120.5 / 147.7 / 7.5
/ 9.1 ms; each row PASS. The plan predicted 15-17 and 11.5-13 at 8K (the 4096
row is 0.4 over) and 1080p unchanged within 2% of 379.4 / 405.8 (the 1024 row
is 2.5% under, the 4096 row 5.1% over). With the hook, three runs read
15.1 / 12.5 / 350.7 / 398.7 (`bench8_after_run{1,2,3}.txt`).

**Tint polarity, in the app** (with the hook; the tint path is unchanged by
...
- No page hook on stb: measured worse in the app, above.
...
session persistence; vertex handles on a path; a Bezier path.

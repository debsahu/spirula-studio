...
(`MaskSession.cpp:650-661`, `if (r.empty()) return {};`) and `upload_rect`
...
- [x] **`<`, `>` and the slider change frames; a dirty frame reports "Saved"
  ...
  **not** independently exercised; `go_to()` is the single function behind
  all three (`MaskPanel.cpp:225`, `:229`, `:231`), so this is inferred from
  the one call tested.
...
`_status` as it publishes the loaded frame (`MaskSession.cpp:285`, inside the
`enqueue` lambda that starts at `:265`), and `pump()` calls
`_livewire.reset()` when it installs that frame on the UI thread (`:337`).
...
(`MaskSession.h:159-160`) address the one float. The slider, `[`/`]` and
...
`MaskPanel.cpp:237-251`, on the **frame-navigation row**, shown only under the
...
(`MaskPanel.cpp:252-255`), plus a `help_on_hover` sentence of the kind `save` /
...
`wheel_brush` join it in `MaskSession.cpp:668-693`, the file
...
- **The open frame's pixels are co-owned.** `_rgb` is a
  `shared_ptr<const vector<uint8_t>>` (`MaskSession.h:403`) and a job holds
...
- **Every job is stamped with a generation and the frame key**
  ...
  at the one assignment of `_doc`, in `pump()` (`MaskSession.cpp:314`), so any

...
| D74 | Rolling shutter is a per-image twist derived from the trajectory, not a solver column: the readout time is one scalar per camera found by a 1-D search on the cost, so the dof tiers and the Schur structure are untouched |
| D75 | A per-image twist is a 6x6 solve with the geometry held, not six more columns: only that image's observations see it, so block coordinate descent buys it for no change to the reduced camera system -- and it needs a prior toward the trajectory, because a free twist absorbs pose error instead of the shutter |

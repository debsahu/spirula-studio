...
SS_MSG(flare_not_fisheye,
    EN("Sun ghost removal skipped for {0}: it works on a fisheye lens's own frames only"),
    JA("{0} の太陽ゴースト除去をスキップしました。魚眼レンズそのもののフレームにしか使えません"),
    ZH_HANS("已跳过 {0} 的太阳鬼影去除：它只适用于鱼眼镜头自身的帧"),
    ZH_HANT("已略過 {0} 的太陽鬼影去除：它只適用於魚眼鏡頭本身的影格"),
    KO("{0} 의 태양 고스트 제거를 건너뜁니다. 어안 렌즈 자체의 프레임에만 쓸 수 있습니다"),
    DE("Entfernen der Sonnen-Geisterbilder für {0} übersprungen: es wirkt nur auf die eigenen Bilder eines Fischaugenobjektivs"),
    FR("Suppression des images fantômes du soleil ignorée pour {0} : elle ne s'applique qu'aux images propres d'un objectif fisheye"),
    ES("Eliminación de fantasmas del sol omitida para {0}: solo funciona con los fotogramas propios de un objetivo ojo de pez"),
    PT("Remoção de fantasmas do sol ignorada para {0}: ela só funciona nos quadros da própria lente olho de peixe"),
    IT("Rimozione dei fantasmi del sole saltata per {0}: funziona solo sui fotogrammi di un obiettivo fisheye"),
    NL("Verwijderen van zonnespiegelingen overgeslagen voor {0}: het werkt alleen op de eigen beelden van een fisheye-lens"),
    RU("Удаление солнечных бликов пропущено для {0}: оно работает только с кадрами самого объектива «рыбий глаз»"),
    TR("{0} için güneş hayaleti giderme atlandı: yalnızca balık gözü merceğin kendi karelerinde çalışır"));

SS_MSG(flare_unsupported_log,
    EN("Sun ghost removal skipped for {0}: its log profile cannot be decoded here"),
    JA("{0} の太陽ゴースト除去をスキップしました。このログプロファイルはここではデコードできません"),
    ZH_HANS("已跳过 {0} 的太阳鬼影去除：此处无法解码它的对数曲线"),
    ZH_HANT("已略過 {0} 的太陽鬼影去除：此處無法解碼它的對數曲線"),
    KO("{0} 의 태양 고스트 제거를 건너뜁니다. 이 로그 프로파일은 여기서 디코딩할 수 없습니다"),
    DE("Entfernen der Sonnen-Geisterbilder für {0} übersprungen: sein Log-Profil lässt sich hier nicht dekodieren"),
    FR("Suppression des images fantômes du soleil ignorée pour {0} : son profil log ne peut pas être décodé ici"),
    ES("Eliminación de fantasmas del sol omitida para {0}: su perfil logarítmico no se puede decodificar aquí"),
    PT("Remoção de fantasmas do sol ignorada para {0}: o perfil log dele não pode ser decodificado aqui"),
    IT("Rimozione dei fantasmi del sole saltata per {0}: il suo profilo log non si può decodificare qui"),
    NL("Verwijderen van zonnespiegelingen overgeslagen voor {0}: het log-profiel kan hier niet worden gedecodeerd"),
    RU("Удаление солнечных бликов пропущено для {0}: его лог-профиль здесь не декодируется"),
    TR("{0} için güneş hayaleti giderme atlandı: log profili burada çözülemiyor"));

SS_MSG(flare_assume_709,
    EN("{0}: colour mode not readable; sun ghost removal assumes the BT.709 curve"),
    JA("{0}: カラーモードを読めません。太陽ゴースト除去は BT.709 カーブを仮定します"),
    ZH_HANS("{0}：无法读取色彩模式；太阳鬼影去除假定为 BT.709 曲线"),
    ZH_HANT("{0}：無法讀取色彩模式；太陽鬼影去除假定為 BT.709 曲線"),
    KO("{0}: 색상 모드를 읽을 수 없어 태양 고스트 제거는 BT.709 커브로 가정합니다"),
    DE("{0}: Farbmodus nicht lesbar; das Entfernen der Sonnen-Geisterbilder nimmt die BT.709-Kurve an"),
    FR("{0} : mode couleur illisible ; la suppression des images fantômes du soleil suppose la courbe BT.709"),
    ES("{0}: no se puede leer el modo de color; la eliminación de fantasmas del sol supone la curva BT.709"),
    PT("{0}: não foi possível ler o modo de cor; a remoção de fantasmas do sol assume a curva BT.709"),
    IT("{0}: modalità colore non leggibile; la rimozione dei fantasmi del sole assume la curva BT.709"),
    NL("{0}: kleurmodus niet leesbaar; het verwijderen van zonnespiegelingen gaat uit van de BT.709-curve"),
    RU("{0}: цветовой режим не читается; удаление солнечных бликов считает кривую BT.709"),
    TR("{0}: renk modu okunamıyor; güneş hayaleti giderme BT.709 eğrisini varsayıyor"));

SS_MSG(flare_frame,
    EN("Sun ghosts removed from {0} -- ghosts: {1}, pixels changed: {2}"),
    JA("{0} から太陽ゴーストを除去しました -- ゴースト: {1}、変更したピクセル: {2}"),
    ZH_HANS("已从 {0} 去除太阳鬼影 -- 鬼影：{1}，改动的像素：{2}"),
    ZH_HANT("已從 {0} 去除太陽鬼影 -- 鬼影：{1}，改動的像素：{2}"),
    KO("{0} 에서 태양 고스트를 제거했습니다 -- 고스트: {1}, 바뀐 픽셀: {2}"),
    DE("Sonnen-Geisterbilder aus {0} entfernt -- Geisterbilder: {1}, geänderte Pixel: {2}"),
    FR("Images fantômes du soleil retirées de {0} -- fantômes : {1}, pixels modifiés : {2}"),
    ES("Fantasmas del sol eliminados de {0} -- fantasmas: {1}, píxeles cambiados: {2}"),
    PT("Fantasmas do sol removidos de {0} -- fantasmas: {1}, pixels alterados: {2}"),
    IT("Fantasmi del sole rimossi da {0} -- fantasmi: {1}, pixel modificati: {2}"),
    NL("Zonnespiegelingen verwijderd uit {0} -- spiegelingen: {1}, gewijzigde pixels: {2}"),
    RU("Солнечные блики удалены из {0} -- бликов: {1}, изменено пикселей: {2}"),
    TR("{0} içinden güneş hayaletleri giderildi -- hayalet: {1}, değişen piksel: {2}"));

SS_MSG(flare_failed,
    EN("Sun ghost removal failed on {0}: {1}"),
    JA("{0} の太陽ゴースト除去に失敗しました: {1}"),
    ZH_HANS("{0} 的太阳鬼影去除失败：{1}"),
    ZH_HANT("{0} 的太陽鬼影去除失敗：{1}"),
    KO("{0} 의 태양 고스트 제거에 실패했습니다: {1}"),
    DE("Entfernen der Sonnen-Geisterbilder für {0} fehlgeschlagen: {1}"),
    FR("La suppression des images fantômes du soleil a échoué pour {0} : {1}"),
    ES("La eliminación de fantasmas del sol falló en {0}: {1}"),
    PT("A remoção de fantasmas do sol falhou em {0}: {1}"),
    IT("Rimozione dei fantasmi del sole non riuscita per {0}: {1}"),
    NL("Verwijderen van zonnespiegelingen mislukt voor {0}: {1}"),
    RU("Не удалось удалить солнечные блики в {0}: {1}"),
    TR("{0} için güneş hayaleti giderme başarısız oldu: {1}"));

SS_MSG(flare_summary,
    EN("Sun ghost removal on {0} -- frames changed: {1} of {2}"),
    JA("{0} の太陽ゴースト除去 -- 変更したフレーム: {1} / {2}"),
    ZH_HANS("{0} 的太阳鬼影去除 -- 改动的帧：{1} / {2}"),
    ZH_HANT("{0} 的太陽鬼影去除 -- 改動的影格：{1} / {2}"),
    KO("{0} 의 태양 고스트 제거 -- 바뀐 프레임: {1} / {2}"),
    DE("Entfernen der Sonnen-Geisterbilder in {0} -- geänderte Bilder: {1} von {2}"),
    FR("Suppression des images fantômes du soleil dans {0} -- images modifiées : {1} sur {2}"),
    ES("Eliminación de fantasmas del sol en {0} -- fotogramas cambiados: {1} de {2}"),
    PT("Remoção de fantasmas do sol em {0} -- quadros alterados: {1} de {2}"),
    IT("Rimozione dei fantasmi del sole in {0} -- fotogrammi modificati: {1} su {2}"),
    NL("Verwijderen van zonnespiegelingen in {0} -- gewijzigde beelden: {1} van {2}"),
    RU("Удаление солнечных бликов в {0} -- изменено кадров: {1} из {2}"),
    TR("{0} içinde güneş hayaleti giderme -- değişen kare: {1} / {2}"));

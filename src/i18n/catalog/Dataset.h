...
SS_MSG(flare_removal,
    EN("Remove sun ghosts (fisheye clips)"),
    JA("太陽ゴーストを除去（魚眼クリップ）"),
    ZH_HANS("去除太阳鬼影（鱼眼片段）"),
    ZH_HANT("去除太陽鬼影（魚眼片段）"),
    KO("태양 고스트 제거 (어안 클립)"),
    DE("Sonnen-Geisterbilder entfernen (Fischaugen-Clips)"),
    FR("Retirer les images fantômes du soleil (clips fisheye)"),
    ES("Eliminar fantasmas del sol (clips de ojo de pez)"),
    PT("Remover fantasmas do sol (clipes olho de peixe)"),
    IT("Rimuovi i fantasmi del sole (clip fisheye)"),
    NL("Zonnespiegelingen verwijderen (fisheye-clips)"),
    RU("Удалять солнечные блики (клипы «рыбий глаз»)"),
    TR("Güneş hayaletlerini gider (balık gözü klipler)"));

SS_MSG(flare_removal_help,
    EN("When the sun is in a lens, that lens records soft rounded reflections of it that the scene never had. This finds them near the line from the lens centre through the sun, fits each one, and subtracts it in linear light from the frames, before masking and reconstruction read them. It never touches pixels outside a fitted reflection, never touches the sun itself, and leaves reflections on textured scene alone. For fisheye lens frames only (OSV, INSV); off by default."),
    JA("太陽がレンズに入ると、そのレンズには実際の景色にない、柔らかく角の丸い太陽の反射が写ります。この機能はレンズ中心から太陽を通る線の近くでそれを見つけ、一つずつ当てはめて、マスクや再構成が読む前にフレームからリニア光で差し引きます。当てはめた反射の外側のピクセルや太陽そのものには触れず、模様のある景色の上の反射はそのままにします。魚眼レンズのフレーム（OSV、INSV）専用で、既定ではオフです。"),
    ZH_HANS("太阳进入镜头时，该镜头会记录下场景中本不存在的、柔和的圆角太阳反射。此功能在从镜头中心穿过太阳的直线附近找到它们，逐个拟合，并在遮罩和重建读取之前，以线性光从帧中减去。它不会改动拟合反射之外的像素，不会改动太阳本身，也不处理落在有纹理景物上的反射。仅用于鱼眼镜头的帧（OSV、INSV）；默认关闭。"),
    ZH_HANT("太陽進入鏡頭時，該鏡頭會記錄下場景中本不存在的、柔和的圓角太陽反射。此功能在從鏡頭中心穿過太陽的直線附近找到它們，逐一擬合，並在遮罩和重建讀取之前，以線性光從影格中減去。它不會改動擬合反射之外的像素，不會改動太陽本身，也不處理落在有紋理景物上的反射。僅用於魚眼鏡頭的影格（OSV、INSV）；預設關閉。"),
    KO("태양이 렌즈에 들어오면 그 렌즈에는 장면에 없던 부드럽고 모서리가 둥근 태양 반사가 기록됩니다. 이 기능은 렌즈 중심에서 태양을 지나는 선 근처에서 그 반사를 찾아 하나씩 맞추고, 마스크와 재구성이 읽기 전에 프레임에서 선형 광으로 뺍니다. 맞춘 반사 밖의 픽셀과 태양 자체는 건드리지 않고, 질감이 있는 장면 위의 반사는 그대로 둡니다. 어안 렌즈 프레임(OSV, INSV) 전용이며 기본값은 꺼짐입니다."),
    DE("Steht die Sonne in einem Objektiv, zeichnet es weiche, abgerundete Spiegelungen von ihr auf, die es in der Szene nie gab. Diese Option sucht sie nahe der Linie von der Objektivmitte durch die Sonne, passt jede einzeln an und zieht sie in linearem Licht von den Bildern ab, bevor Maskierung und Rekonstruktion sie lesen. Pixel außerhalb einer angepassten Spiegelung und die Sonne selbst bleiben unberührt, ebenso Spiegelungen auf strukturierter Szene. Nur für Bilder von Fischaugenobjektiven (OSV, INSV); standardmäßig aus."),
    FR("Quand le soleil est dans un objectif, celui-ci enregistre des reflets doux aux coins arrondis que la scène n'avait pas. Cette option les cherche près de la droite qui va du centre de l'objectif au soleil, ajuste chacun et le soustrait des images en lumière linéaire, avant que le masquage et la reconstruction ne les lisent. Elle ne touche jamais les pixels hors d'un reflet ajusté, ni le soleil lui-même, et laisse les reflets posés sur une scène texturée. Pour les images d'objectif fisheye seulement (OSV, INSV) ; désactivée par défaut."),
    ES("Cuando el sol está en un objetivo, este registra reflejos suaves de esquinas redondeadas que la escena nunca tuvo. Esta opción los busca cerca de la línea que va del centro del objetivo al sol, ajusta cada uno y lo resta de los fotogramas en luz lineal, antes de que el enmascarado y la reconstrucción los lean. Nunca toca píxeles fuera de un reflejo ajustado ni el propio sol, y deja los reflejos que caen sobre escena con textura. Solo para fotogramas de objetivo ojo de pez (OSV, INSV); desactivada por defecto."),
    PT("Quando o sol está em uma lente, ela registra reflexos suaves de cantos arredondados que a cena nunca teve. Esta opção os procura perto da linha que vai do centro da lente ao sol, ajusta cada um e o subtrai dos quadros em luz linear, antes que o mascaramento e a reconstrução os leiam. Ela nunca mexe em pixels fora de um reflexo ajustado nem no próprio sol, e deixa em paz os reflexos sobre cena com textura. Só para quadros de lente olho de peixe (OSV, INSV); desligada por padrão."),
    IT("Quando il sole è in un obiettivo, questo registra riflessi morbidi dagli angoli arrotondati che la scena non aveva. Questa opzione li cerca vicino alla retta che va dal centro dell'obiettivo al sole, adatta ciascuno e lo sottrae dai fotogrammi in luce lineare, prima che il mascheramento e la ricostruzione li leggano. Non tocca mai pixel fuori da un riflesso adattato né il sole stesso, e lascia i riflessi che cadono su una scena con texture. Solo per fotogrammi di obiettivi fisheye (OSV, INSV); disattivata per impostazione predefinita."),
    NL("Staat de zon in een lens, dan legt die lens zachte, afgeronde spiegelingen van de zon vast die de scène nooit had. Deze optie zoekt ze bij de lijn van het lensmidden door de zon, past ze een voor een aan en trekt ze in lineair licht van de beelden af, voordat maskering en reconstructie ze lezen. Pixels buiten een aangepaste spiegeling en de zon zelf blijven onaangeroerd, net als spiegelingen op een scène met textuur. Alleen voor beelden van fisheye-lenzen (OSV, INSV); standaard uit."),
    RU("Когда солнце попадает в объектив, он записывает мягкие отражения солнца со скруглёнными углами, которых в сцене не было. Эта функция ищет их у линии от центра объектива через солнце, подгоняет каждое и вычитает его из кадров в линейном свете, до того как их прочтут маскирование и реконструкция. Пиксели вне подогнанного отражения и само солнце не меняются, отражения на текстурированной сцене тоже остаются. Только для кадров объектива «рыбий глаз» (OSV, INSV); по умолчанию выключено."),
    TR("Güneş bir merceğe girdiğinde o mercek, sahnede hiç olmayan, yumuşak ve köşeleri yuvarlak güneş yansımaları kaydeder. Bu seçenek onları mercek merkezinden güneşe giden doğrunun yakınında bulur, her birini uydurur ve maskeleme ile yeniden yapılandırma okumadan önce karelerden doğrusal ışıkta çıkarır. Uydurulan yansımanın dışındaki piksellere ve güneşin kendisine asla dokunmaz, dokulu sahne üzerindeki yansımaları da olduğu gibi bırakır. Yalnızca balık gözü mercek kareleri için (OSV, INSV); varsayılan olarak kapalı."));

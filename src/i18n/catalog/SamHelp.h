...
SS_MSG(xh_flare_removal,
    EN("subtract the sun's lens ghosts from each fisheye frame after it is written (off by default; not with --model)"),
    JA("書き出した魚眼フレームごとに太陽のレンズゴーストを差し引く（既定はオフ。--model とは併用不可）"),
    ZH_HANS("每个鱼眼帧写出后减去太阳的镜头鬼影（默认关闭；不能与 --model 同用）"),
    ZH_HANT("每個魚眼影格寫出後減去太陽的鏡頭鬼影（預設關閉；不能與 --model 同用）"),
    KO("어안 프레임을 쓴 뒤마다 태양의 렌즈 고스트를 뺍니다(기본값 꺼짐, --model 과 함께 쓸 수 없음)"),
    DE("zieht nach dem Schreiben jedes Fischaugenbildes die Objektiv-Geisterbilder der Sonne ab (Vorgabe: aus; nicht mit --model)"),
    FR("soustrait les images fantômes du soleil de chaque image fisheye une fois écrite (désactivé par défaut ; pas avec --model)"),
    ES("resta los fantasmas del sol de cada fotograma de ojo de pez una vez escrito (desactivado por defecto; no con --model)"),
    PT("subtrai os fantasmas do sol de cada quadro olho de peixe depois de gravado (desligado por padrão; não com --model)"),
    IT("sottrae i fantasmi del sole da ogni fotogramma fisheye dopo averlo scritto (disattivato per impostazione predefinita; non con --model)"),
    NL("trekt de lensspiegelingen van de zon af van elk fisheye-beeld nadat het is geschreven (standaard uit; niet met --model)"),
    RU("вычитать солнечные блики объектива из каждого кадра «рыбий глаз» после записи (по умолчанию выключено; не вместе с --model)"),
    TR("her balık gözü kare yazıldıktan sonra güneşin mercek hayaletlerini çıkarır (varsayılan kapalı; --model ile olmaz)"));

SS_MSG(cmd_flare,
    EN("Subtract the sun's lens ghosts from every fisheye frame under <images>, in place, before anything else reads them. A frame without a fitted ghost is left untouched, and each file is done once."),
    JA("<images> の下にある魚眼フレームすべてから、太陽のレンズゴーストをその場で差し引きます。ほかの処理が読む前に行ってください。ゴーストを当てはめられなかったフレームには触れず、各ファイルは一度だけ処理します。"),
    ZH_HANS("在其他步骤读取之前，就地从 <images> 下的每个鱼眼帧中减去太阳的镜头鬼影。没有拟合到鬼影的帧保持不变，每个文件只处理一次。"),
    ZH_HANT("在其他步驟讀取之前，就地從 <images> 下的每個魚眼影格中減去太陽的鏡頭鬼影。沒有擬合到鬼影的影格保持不變，每個檔案只處理一次。"),
    KO("다른 처리가 읽기 전에 <images> 아래의 모든 어안 프레임에서 태양의 렌즈 고스트를 그 자리에서 뺍니다. 고스트를 맞추지 못한 프레임은 그대로 두고, 각 파일은 한 번만 처리합니다."),
    DE("Zieht die Objektiv-Geisterbilder der Sonne direkt von jedem Fischaugenbild unter <images> ab, bevor etwas anderes sie liest. Ein Bild ohne angepasstes Geisterbild bleibt unberührt, und jede Datei wird nur einmal bearbeitet."),
    FR("Soustrait sur place les images fantômes du soleil de chaque image fisheye sous <images>, avant que quoi que ce soit ne les lise. Une image sans fantôme ajusté reste intacte, et chaque fichier n'est traité qu'une fois."),
    ES("Resta en su sitio los fantasmas del sol de cada fotograma de ojo de pez bajo <images>, antes de que nada más los lea. Un fotograma sin fantasma ajustado queda intacto, y cada archivo se procesa una sola vez."),
    PT("Subtrai no próprio arquivo os fantasmas do sol de cada quadro olho de peixe em <images>, antes que qualquer outra etapa os leia. Um quadro sem fantasma ajustado fica intacto, e cada arquivo é processado uma única vez."),
    IT("Sottrae sul posto i fantasmi del sole da ogni fotogramma fisheye sotto <images>, prima che altro li legga. Un fotogramma senza fantasma adattato resta intatto, e ogni file viene trattato una sola volta."),
    NL("Trekt de lensspiegelingen van de zon ter plekke af van elk fisheye-beeld onder <images>, voordat iets anders ze leest. Een beeld zonder aangepaste spiegeling blijft onaangeroerd, en elk bestand wordt maar één keer bewerkt."),
    RU("Вычитает солнечные блики объектива на месте из каждого кадра «рыбий глаз» в <images>, до того как их прочтёт что-то ещё. Кадр без подогнанного блика не меняется, каждый файл обрабатывается один раз."),
    TR("<images> altındaki her balık gözü kareden, başka bir şey okumadan önce, güneşin mercek hayaletlerini yerinde çıkarır. Uydurulmuş hayaleti olmayan kareye dokunulmaz ve her dosya yalnızca bir kez işlenir."));

SS_MSG(fl_curve,
    EN("how the frames' code values become light: dlogm for a D-Log M clip, bt709 for everything else"),
    JA("フレームのコード値を光に戻す方法: D-Log M のクリップは dlogm、それ以外は bt709"),
    ZH_HANS("帧的码值如何还原为光：D-Log M 片段用 dlogm，其余用 bt709"),
    ZH_HANT("影格的碼值如何還原為光：D-Log M 片段用 dlogm，其餘用 bt709"),
    KO("프레임의 코드 값을 빛으로 되돌리는 방법: D-Log M 클립은 dlogm, 나머지는 bt709"),
    DE("wie die Codewerte der Bilder zu Licht werden: dlogm für einen D-Log-M-Clip, bt709 für alles andere"),
    FR("comment les valeurs de code des images redeviennent de la lumière : dlogm pour un clip D-Log M, bt709 pour le reste"),
    ES("cómo los valores de código de los fotogramas se vuelven luz: dlogm para un clip D-Log M, bt709 para todo lo demás"),
    PT("como os valores de código dos quadros viram luz: dlogm para um clipe D-Log M, bt709 para todo o resto"),
    IT("come i valori di codice dei fotogrammi tornano luce: dlogm per una clip D-Log M, bt709 per tutto il resto"),
    NL("hoe de codewaarden van de beelden weer licht worden: dlogm voor een D-Log M-clip, bt709 voor al het andere"),
    RU("как кодовые значения кадров переводятся в свет: dlogm для клипа D-Log M, bt709 для всего остального"),
    TR("karelerin kod değerlerinin ışığa nasıl döndüğü: D-Log M klip için dlogm, geri kalan her şey için bt709"));
